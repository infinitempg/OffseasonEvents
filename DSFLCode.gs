function onOpen() {
  ...
  ui.createMenu('DSFL Draft Tools')
//  .addItem('Generate Headers', 'buildHeader')
  .addItem('Make Board','makeBoard')
//  .addItem('Make Best List', 'makeBestList')
  .addToUi();
}
 
function buildHeader(length,pickIndex) {
...
//  var bestSheet = ss.getSheetByName('bestList')
  ...
  var pickSheet = ss.getSheetByName('picks')
  var draftCardSheet = ss.getSheetByName('pickCSV')
...
// Getting values
  ...
  buildHeader(pickArray.length,pickIndex)
